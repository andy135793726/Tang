...
public class RoundedRect extends Shapes {
	...
	public BufferedImage paint(BufferedImage bi, Graphics g, JComponent canvas, MouseEvent e) {
		...
		int arcWidth   = width >= 90 ? 30 : width/3;
		int arcHeight  = height>= 90 ? 30 : height/3;
		
		
		((Graphics2D)g).setStroke(b);

		g.drawRoundRect( (x1 >= x0? x0 : x1), (y1 <= y0? y1 : y0), width, height, arcWidth, arcHeight);
